...
description: 
  Learn how to make use of the Trundler API in R 
---
...
[Trundler](https://www.trundler.dev/#about) is a curated repository of historical pricing data from major retailers. Data from the Trundler [API](https://api.trundler.dev/) can be pulled into R easily with this package. Trundler is designed for use by suppliers who want to monitor mark-ups on their products, retailers who want to understand the pricing strategies of their competitors or regulators who want to monitor regulatory compliance. 

This vignette describes the use of central Trundler functions `retailer()`, `products()` and `product_prices()` as well as some examples to get you started on your journey into retail transparency. 

Browse the [blog](https://www.trundler.dev/blog/) to see additional examples of Trundler in action.  
...
## Usage 
To load the trundler package in R:
...
## Set API Key 
To access the full API you’ll first need to obtain a key from the [Trundler API](https://api.trundler.dev/).
...
Get price history data for a specific product of interest:
```{r p_hist}
...
Here we have utilised [tidyverse](https://www.tidyverse.org/) tools to compare the prices of similar surgical masks sold by two South African retailers. 

To find the products and match them up to retailers of interest:
```{r mask-products, cache=TRUE}
library(tidyverse)
...
To obtain tidy historical pricing data for both retailers: 
```{r mask-prices, cache=TRUE}
